...
| `PaymentSuccessPage`는 완료 응답을 받으면 확정 요청과 복귀 정보를 삭제하고 결과는 React 상태에만 둔다. URL도 정리하므로 완료 화면을 새로고침하면 정상 결제인데도 `결제 확정 실패` 화면이 나온다. | 처리 중 확정 요청과 완료 조회 정보를 분리한다. 같은 고객 세션의 완료 결제 번호로 기존 상태 조회 API를 사용해 결과와 내역 이동을 복원하고, 완료 화면 재조회 때문에 새 결제를 막거나 승인 요청을 반복하지 않는다. |
