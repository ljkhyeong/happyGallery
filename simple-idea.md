...
| 회원 장바구니의 `DefaultCartService.updateItemQty`는 감소 요청에도 현재 옵션과 합산 재고를 검증한다. 5개를 담은 뒤 재고가 2개로 줄면 화면의 `−` 버튼은 4개를 요청하므로 실패하고, 계속 눌러도 수량을 줄이지 못한다. 판매 중지·옵션 변경도 감소를 막는다. | 비회원과 같이 감소 요청에는 소유권·수량 범위만 확인하고 추가·증가에 현재 상품·옵션·합산 재고 검증을 적용한다. 회원 화면도 재고 초과와 판매 중지를 구분해 수량 조정 또는 삭제를 안내한다. |
| `ProductPurchaseOptions`는 선택할 때 단가와 표시명을 `PurchaseLine`에 저장하고 `ProductDetailPage`는 이 단가로 합계를 계산한다. 장바구니 담기의 최신 상품 조회 등으로 기본가·옵션 추가금이 갱신되어도 기존 선택 행과 합계는 이전 가격을 표시하며, 서버 결제 준비는 현재 가격을 사용한다. | 선택 행에는 조합·입력·수량을 유지하고 단가·표시명은 현재 상품 응답에서 계산한다. 가격이나 옵션 구성이 달라지면 고객이 변경 내용을 확인할 수 있게 안내하고, 서버의 가격 확정과 결제 후 구매 당시 스냅샷은 유지한다. |
| 서버 `CartItem.lineKey`는 직접입력의 옵션 키보다 관리자 표시 순서인 `sortOrder`를 먼저 정렬한다. 직접입력 두 그룹의 순서만 바꾸어도 같은 상품·조합·각인 문구가 다른 키가 되어 새 장바구니 행으로 담긴다. Java 25에서 실제 도메인 클래스로 키 불일치를 확인했다. | 표시 순서를 항목 식별에서 제외하고 안정적인 옵션 키 순서로 계산한다. 기존 키 전환 시 이미 나뉜 동일 입력 행과 결제 준비의 항목 ID 참조를 함께 처리하며, 기존 V162를 수정하지 않고 후속 Flyway로 전환한다. |
