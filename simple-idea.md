...
| 상품 상세·비회원 장바구니·주문서는 직접입력 값을 구분자로 이어 항목 키를 만든다. 구분자가 들어간 한 문구와 여러 옵션 입력이 같은 키로 계산돼 다른 문구가 합쳐질 수 있다. 서버 `CartItem.lineKey`와 병합 요청 해시도 같은 종류의 문자열 결합을 사용한다. | 옵션 키와 값을 정렬된 배열 등 구조가 보존되는 형태로 직렬화해 식별한다. 화면의 중복 키 생성 함수를 공통화하고 서버의 장바구니 키·병합 요청 해시도 함께 수정한다. 기존 저장 키의 전환을 포함하되 정상 각인 문자를 금지하지 않는다. |
| `useGuestCart`의 추가·수량 변경은 행별 1~99개만 확인한다. 상세 화면에서 제한해도 장바구니에서 다시 늘리거나 재방문해 추가할 수 있고, 다른 각인 문구로 같은 조합 100개도 저장된다. 주문제작은 초과 저장 뒤 품절로 표시하며 기성품은 공개 응답에 실제 재고 수량도 없다. | 비회원 장바구니의 추가·수량 변경 시 기존 항목을 포함한 상품·조합별 합산 수량에 재고와 99개 한도를 적용한다. 기성품 판매 가능 수량은 API 계약에 반영하고, 재고 감소로 초과된 항목은 수량 조정 안내를 표시한다. 서버의 최종 재고 검증은 유지한다. |
| `ProductDetailPage`의 여러 옵션 담기는 항목마다 별도 요청을 보낸다. 앞 항목 저장 후 뒤 항목이 기존 장바구니 수량·재고 때문에 거절되면 일부만 담기며, 다시 누를 때 앞 항목이 중복 추가된다. | 기존 다건 장바구니 병합 API와 멱등 처리를 재사용해 선택한 항목을 한 번에 검증·저장한다. 비회원도 한 번의 장바구니 잠금·저장으로 처리해 실패 시 일부 항목만 남지 않게 한다. |
