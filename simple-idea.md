...
| `ProductPurchaseOptions`는 각인 문구가 다른 행의 재고를 각각 제한한다. 같은 옵션 조합 재고가 2개여도 문구 A 2개와 문구 B 2개를 선택할 수 있고, 바로구매는 서버 prepare에서 거절된다. 장바구니 담기는 행별 요청이어서 앞선 행만 추가될 수 있다. | 같은 `productVariantId`의 선택 수량을 합산해 옵션 추가와 수량 변경 때 남은 수량을 표시하고 제한한다. 서버의 기존 조합별 합산·최신 재고 검증은 유지한다. |
