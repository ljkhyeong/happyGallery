...
| 스마트스토어 문의 조회가 실패하면 `SmartStoreInquirySection`이 오류만 반환해 상품 문의·주문/배송 문의 탭과 조회 조건도 사라진다. | 탭과 조회 조건은 유지하고 목록 영역에 오류와 재시도 버튼을 표시해 다른 문의 조회를 계속할 수 있게 한다. |
| 스마트스토어 주문·배송 문의 답변은 수정할 수 있지만 상품 문의는 답변을 표시하기만 한다. 서버는 이미 등록·수정 공용 PUT API를 호출한다. | 상품 문의에도 기존 답변을 불러오는 수정·저장·취소 화면을 연결한다. [공식 답변 등록·수정 API](https://apicenter.commerce.naver.com/docs/commerce-api/current/create-or-update-answer-contents)를 그대로 사용한다. |
| 결제 실패 화면 `PaymentFailPage`는 홈·상품 목록 링크만 제공해 예약금이나 8회권 결제를 취소한 고객도 원래 구매 화면으로 돌아갈 수 없다. | 기존 결제 복귀 정보와 고객 세션 확인을 사용해 원래 주문서·예약·8회권 화면으로 돌아가는 버튼을 제공한다. |
