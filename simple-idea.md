...
| 스마트스토어 문의 조회는 서버에서 최근 30일로 고정하고 화면은 100건만 요청한다. 기간 선택과 다음 페이지가 없어 오래된 문의나 조회 한도를 넘는 답변 이력을 찾기 어렵다. | 네이버 API가 허용하는 범위에서 조회 기간과 페이지 이동을 제공한다. 조회 조건은 상품 문의·주문/배송 문의별로 유지하고 미답변 필터와 함께 사용한다. |
