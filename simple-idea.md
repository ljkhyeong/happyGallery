...
| `payment/flow.ts`의 0원 분기는 결제 복구 정보를 저장하기 전에 각 화면의 confirm을 호출한다. 주문서·장바구니·8회권 예약에서 confirm 응답이 유실되면 오류만 표시하고 다음 클릭은 새 prepare를 시작한다. | 0원 결제도 현재 고객 세션에 결제 번호와 확정 요청을 보관하고, 결과가 불명확하면 기존 번호로 상태 조회·재확인을 제공한다. 기존 거래의 완료 여부를 확인하기 전에는 새 결제를 준비하지 않는다. |
