...
| 비회원 결제 준비는 인증코드를 소비하지만 `AuthGateModal`의 `guestVerified`와 코드가 그대로 남는다. 코드가 틀렸거나 결제창을 닫은 뒤 모달을 다시 열어도 인증 입력으로 돌아갈 수 없다. 비회원 주문서도 소비된 코드를 계속 제출할 수 있다. | 비회원 결제 준비 이후 실패와 인증 실패를 구분해 재인증으로 안내한다. 주문·예약 선택과 이름은 유지하고 코드만 초기화하며, 재발송 때도 상위 화면의 기존 코드와 진행 가능 상태를 함께 해제한다. |
| 상품 상세 `ProductDetailPage`의 회원 바로구매에는 쿠폰·적립금 선택과 요청 값이 없다. 같은 회원 주문인데 장바구니·주문서에서만 혜택을 적용할 수 있다. | 기존 `MemberOrderBenefits`를 바로구매에도 재사용하고 최종 결제액을 표시한다. 전액 할인은 0원 결제 복구 경로를 공유하며 서버의 기존 혜택 계산·예약을 그대로 사용한다. |
