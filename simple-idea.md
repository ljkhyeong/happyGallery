...
| 결제창을 닫고 구매 화면으로 돌아와도 서버 결제 시도는 `PENDING`이며 쿠폰·적립금 예약은 30분 만료 처리까지 남는다. 새 prepare는 같은 쿠폰이나 예약 중인 적립금을 사용할 수 없다. | 본인 소유의 미승인 결제 시도를 명시적으로 종료하고 같은 트랜잭션에서 혜택 예약을 해제한다. 승인 진행·대사·보상 중인 건은 해제하지 않는다. 화면 복귀와 서버 결제 종료는 구분한다. |
| 영수증 링크는 결제 완료 화면과 비회원 결제 상태 화면에만 있고 회원 주문·예약·8회권 내역에는 없다. | 내 거래 상세에서 소유권을 확인한 뒤 기존 결제 원장의 영수증 링크를 다시 제공한다. 원장 값을 각 도메인에 복제하지 않는다. |
