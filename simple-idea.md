...
| 비회원 옵션 주문을 넘겨받는 `OrderItemsForm`은 선택한 조합과 각인 입력을 요청에는 포함하지만 화면에는 상품명·수량·금액·고정 사양만 표시한다. 같은 상품의 다른 옵션이나 문구를 결제 직전에 구분하기 어렵다. | 주문서 품목마다 옵션 그룹명·선택값·직접입력 문구를 표시해 결제 전에 확인하게 한다. 장바구니·주문 상세의 옵션 표시 형식을 맞추고 서버 요청 필드는 그대로 사용한다. |
