...
### 10.5 테스트/비교 코드 작성 규칙
- 테스트 메서드에는 `@DisplayName`을 필수로 사용하고, 테스트 의도를 설명하는 한글 문장으로 작성한다.
- 한 테스트에서 검증 포인트가 2개 이상이면 `SoftAssertions.assertSoftly`를 기본으로 사용해 실패를 한 번에 확인한다.
- nullable 가능성이 있는 객체 비교(예: `String`, `Long`, 토큰/ID)는 `Objects.equals(a, b)`를 기본으로 사용한다.
