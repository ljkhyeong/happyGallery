...
구체적인 AWS 운영 배포 설정값과 정책 예시는 `docs/Idea/0039_AWS_배포_설정_베이스라인/idea.md`를 기준으로 관리한다.
...
현재 workflow는 `AWS_ACCOUNT_ID`를 직접 사용하지 않는다.
실제 값과 OIDC trust / permission policy 예시는 `0039_AWS_배포_설정_베이스라인` 문서를 본다.
