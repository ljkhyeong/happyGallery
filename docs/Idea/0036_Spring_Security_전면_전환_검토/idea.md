...
- 식별자만 필요한 컨트롤러의 `@AdminUserId`, `@CustomerUserId` 계약은 유지하고 argument resolver가 principal을 읽는다. `/api/v1/me`는 principal의 회원 스냅샷을 재사용한다.
...
- Google·Naver 로그인은 기존 OAuth 클라이언트와 서버 세션 `state` 검증 흐름을 유지한다. `spring-boot-starter-oauth2-client`로 전환하지 않는다.
