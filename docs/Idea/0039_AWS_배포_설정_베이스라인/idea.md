...
- RDS MySQL 기본값
- Redis 기본값
- GitHub Actions가 사용하는 Secrets와 Variables
- GitHub Actions OIDC IAM 역할 신뢰 정책
- GitHub Actions OIDC IAM 역할 권한 정책
...
5. RDS MySQL 생성
6. Redis 생성
7. ECS 클러스터, 서비스, ALB 생성
8. GitHub Actions OIDC IAM 역할 권한 정책 완성
9. GitHub Secrets와 Variables 입력

### 메모

- OIDC 공급자와 IAM 역할의 신뢰 정책은 미리 만들어도 된다.
- 하지만 권한 정책 안의 `S3_BUCKET`, `CLOUDFRONT_DISTRIBUTION_ID`, `ECS_CLUSTER`, `ECS_SERVICE`는 실제 리소스가 만들어진 뒤에만 확정할 수 있다.
- 그래서 OIDC 역할은 보통 `신뢰 정책 먼저`, `권한 정책은 리소스 생성 후 최종 반영` 순서로 진행한다.

## 2. ECR 리포지토리 기본값

현재 GitHub Actions 배포 워크플로우는 `ap-northeast-2` 리전에서 백엔드 이미지를 ECR에 올린다.
ECS task definition은 `ARM64` 기준으로 운영하므로 백엔드 이미지는 `docker buildx`로 `linux/arm64` 플랫폼에 맞춰 빌드한다.

- 타입: 비공개 저장소
- 리전: `ap-northeast-2`
- 권장 이름: `happygallery-backend`
- 태그 변경 허용 여부(tag immutability): `MUTABLE`
- 푸시 시 이미지 검사(scan on push): `enabled`
- 암호화(encryption): 기본 `AES-256`

이 프로젝트의 배포 워크플로우는 같은 이미지에 commit SHA 태그와 `latest` 태그를 함께 올린다. 배포 시에는 현재 서비스의 task definition을 기준으로 새 revision을 등록해 commit SHA 이미지를 명시적으로 넣는다.
그래서 `latest`를 계속 갱신할 수 있도록 태그 변경 허용 값은 `MUTABLE`을 사용한다.
...
운영 프론트엔드 정적 파일은 `S3 + CloudFront` 조합을 기준으로 한다.
...
- 공개 접근(public access): `모두 차단`
- 정적 웹사이트 호스팅(static website hosting): `사용 안 함`
- 버전 관리(versioning): `선택`
- 암호화(encryption): `SSE-S3` 또는 기본 `AES-256`

### 이유

- 현재 운영 방식은 `S3 website endpoint`가 아니라 `CloudFront + OAC`로 비공개 버킷을 원본(origin)으로 쓰는 방식이다.
- 그래서 버킷 자체를 공개로 열지 않는다.
- SPA fallback도 S3 정적 웹사이트 호스팅보다 CloudFront 쪽 정책으로 관리하는 편이 더 맞다.

### 생성할 때 체크할 항목

1. 버킷 이름을 전역에서 유일한 이름으로 만든다.
2. `Block all public access`를 유지한다.
3. 필요하면 versioning을 켠다.
4. 버킷 생성 후 CloudFront 원본(origin)으로 연결한다.
5. GitHub Actions variable에는 버킷 이름만 `S3_BUCKET`으로 넣는다.
...
- 기본 원본(origin): 비공개 S3 버킷
- S3 접근 방식: `OAC (Origin Access Control)`
- 추가 원본(origin): `ALB` (`/api/*` 용)
- 뷰어 프로토콜 정책(viewer protocol policy): `Redirect HTTP to HTTPS`
- 허용 메서드(allowed methods)
  - 기본 정적 동작(behavior): `GET, HEAD, OPTIONS`
  - `/api/*` 동작(behavior): `GET, HEAD, OPTIONS, PUT, POST, PATCH, DELETE`
- 캐시 정책(cache policy)
  - 기본 정적 동작(behavior): `CachingOptimized` 계열
  - `/api/*` 동작(behavior): 캐시 비활성 또는 매우 짧은 TTL
- 압축(compression): `enabled`
...
이 방식이면 브라우저 기준으로 프론트엔드와 API가 같은 도메인 아래에 있으므로, 현재 앱의 세션, 쿠키, CORS 가정과 가장 잘 맞는다.
...
1. 사용자 정의 오류 응답(Custom error response)
   - `403 -> /index.html -> 200`
   - `404 -> /index.html -> 200`
2. CloudFront Function / Lambda@Edge
   - 브라우저 라우트만 `index.html`로 다시 쓴다
...
- S3 버킷은 공개로 열지 않는다.
- CloudFront 배포를 만들 때 OAC를 새로 만든다.
- 버킷 정책(bucket policy)은 OAC가 붙은 CloudFront 배포만 읽을 수 있게 제한한다.
...
- 공개 진입 계층과 비공개 앱/데이터 계층을 분리하기 쉽다.
- ECS, RDS, Redis를 비공개 서브넷에 배치하는 구조가 더 자연스럽다.
...
- VPC: `10.0.0.0/16`
- 공개 서브넷 A: `10.0.1.0/24`
- 공개 서브넷 B: `10.0.2.0/24`
- 비공개 서브넷 A: `10.0.11.0/24`
- 비공개 서브넷 B: `10.0.12.0/24`
...
- 당장 개념 검증만 띄우는 목적이면 default VPC도 가능하지만, 운영 기본값으로는 채택하지 않는다.
...
- 리전: `ap-northeast-2`
- DB 이름: `happygallery`
- 권장 식별자(identifier) 예시: `happygallery-prod-mysql`
- 공개 접근(Public access): `No`
- 배치 위치: ECS/ALB와 같은 VPC, 비공개 서브넷
- 저장소 암호화(storage encryption): `enabled`
- 자동 백업(automated backup): `7일 이상`
- 삭제 방지(deletion protection): `enabled`
...
11. DB subnet group은 비공개 서브넷 기준 선택
...
- `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`는 GitHub Secrets가 아니라 ECS task 실행 시점 또는 Secrets Manager/SSM에서 주입한다.

## 8. Redis 기본값

이 프로젝트는 Redis를 회원 세션, 관리자 Bearer 세션, 처리율 제한 저장소로 사용한다.
...
- 리전: `ap-northeast-2`
- 배치 위치: ECS와 같은 VPC, 비공개 서브넷
- 공개 접근(Public access): 없음
- 권장 식별자(identifier) 예시: `happygallery-prod-redis`
- 권장 포트: `6379`
...
7. subnet group은 비공개 서브넷 기준으로 생성
8. security group은 Redis 전용 보안 그룹 선택
...
- Redis 인바운드를 `0.0.0.0/0`로 열지 않는다.
- Redis 인바운드는 ECS 앱 보안 그룹에서만 받는다.

### 메모

- 이 앱은 `REDIS_HOST`, `REDIS_PORT` 환경변수 기준으로 연결한다.
- Redis 인증이나 TLS를 켜면 ECS task 환경변수와 클라이언트 설정을 함께 맞춰야 한다.
- 운영 비밀값은 GitHub Secrets 대신 ECS 실행 시점 또는 Secrets Manager/SSM에서 주입한다.

## 9. ECS 작업 정의(Task Definition) 기본값

운영 앱 실행은 `ECS Fargate` 기준으로 한다.

### 현재 기준

- 시작 방식(launch type): `Fargate`
- OS: `Linux`
- 작업 정의 family 예시: `happygallery-prod-app`
- 컨테이너 포트: `8080`
- 시작 리소스 권장값
  - CPU: `512`
  - Memory: `1024`
...
- ALB: 공개 서브넷
- ECS 서비스: 비공개 서브넷
- ECS task 공인 IP: `disabled`
- 대상 그룹 타입(target group target type): `ip`
...
| 이름 | 용도 |
|------|------|
| `GUEST_TOKEN_HMAC_SECRET` | 비회원 접근 토큰 서명 |
...
- 비밀값은 GitHub Actions가 아니라 ECS task 실행 시점의 Secrets Manager/SSM 주입을 우선한다.
...
## 10. ALB 대상 그룹 헬스 체크 기준

### 현재 기준

- 헬스 체크 경로: `/actuator/health`
- 헬스 체크 포트: `traffic port (8080)`
- matcher: `200`

### 이유

- 기본 설정상 `management.server.port`는 운영 환경에서 `8081`이다.
- 하지만 초기 ECS/ALB 운영에서는 헬스 체크 단순화가 더 중요하므로 `MANAGEMENT_PORT=8080`으로 덮어 actuator를 앱 포트와 합친다.
- 이렇게 하면 ALB 대상 그룹 헬스 체크를 별도 관리 포트 없이 바로 사용할 수 있다.
...
- 나중에 관리 포트를 애플리케이션 포트와 다시 분리하고 싶을 때
- ALB 대신 service connect, service mesh, 내부 헬스 체크 전략을 별도로 둘 때

## 11. GitHub Actions Secrets와 Variables 기본값
...
| 이름 | 용도 |
|------|------|
| `ECR_REPOSITORY` | 백엔드 Docker 이미지 업로드 대상 ECR 저장소 이름 |
| `ECS_CLUSTER` | 백엔드 ECS 클러스터 이름 |
| `ECS_SERVICE` | 백엔드 ECS 서비스 이름 |
| `S3_BUCKET` | 프론트엔드 정적 파일을 동기화할 S3 버킷 이름 |
| `CLOUDFRONT_DISTRIBUTION_ID` | 프론트엔드 배포 후 캐시 무효화 대상 CloudFront 배포 ID |

### 메모

- 현재 deploy workflow는 `AWS_ACCOUNT_ID`를 직접 사용하지 않는다.
- DB 비밀번호, Redis 비밀번호, API 키, 암호화 키 같은 운영 비밀값은 GitHub Secrets에 넣지 않는다.
- 앱 실행 시점 비밀값은 ECS Task Definition과 Secrets Manager 또는 SSM Parameter Store에서 주입한다.

## 12. GitHub Actions OIDC IAM 역할 신뢰 정책 기본값

현재 deploy workflow는 `push -> main`에서만 배포된다.
그래서 신뢰 정책도 우선 `main` 브랜치만 역할을 맡을 수 있게 좁게 잡는다.

현재 저장소 기준 GitHub repository 식별자는 `ljkhyeong/happyGallery`다.
권장 IAM 역할 이름은 `GitHubActionsHappyGalleryDeployRole`로 둔다.
...
## 13. GitHub Actions OIDC IAM 역할 권한 정책 기본값

단일 deploy role을 유지한다면 아래 권한 세트가 현재 workflow와 맞는다.

- ECR 로그인과 이미지 업로드
- ECS 작업 정의 조회, 새 revision 등록, 서비스 배포
- S3 정적 파일 동기화
- CloudFront 캐시 무효화
...
- 이 권한 정책은 ECR, S3, CloudFront, ECS 리소스 이름이 확정된 뒤 최종 반영한다.
- 나중에 프론트엔드와 백엔드 배포 역할을 분리하면 이 정책도 역할별로 나누는 편이 더 안전하다.
- 현재 워크플로우는 task definition 새 revision까지 GitHub Actions가 직접 등록하므로 `ecs:DescribeTaskDefinition`, `ecs:RegisterTaskDefinition`, `iam:PassRole`을 포함해야 한다.
