...
> 최종 채택 기준은 [ADR-0023](../../ADR/0023_관리자_회원_인증_세션_기준선/adr.md) 확인. 이 문서는 전환 배경과 검토 맥락 보존용이다.
...
### 2. 비회원 이력 가져오기(claim) 흐름 회귀 방지

- 로그인 직후 `/my?claim=1`
- guest 성공 화면 → 회원가입/로그인 → 이력 가져오기(claim) 모달 자동 진입
- 전화번호 재인증 후 claim
