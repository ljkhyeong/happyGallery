...
이 구조만으로도 레이어 책임은 어느 정도 나뉘어 있지만,
애플리케이션 서비스가 Spring Data repository나 외부 연동 구현을 직접 아는 구간이 아직 많다.
...
다만 현재 구조는 이미 운영 중이고, 기능 흐름과 통합 테스트도 충분히 쌓여 있으므로
big-bang 재작성으로 헥사고날 구조를 한 번에 강제하는 것은 위험이 크다.

따라서 “전면 재작성”이 아니라,
기존 모듈 구조를 유지한 채 `port / adapter` 경계를 점진적으로 도입하는 전략이 필요하다.
...
즉, 모듈을 갈아엎지 않고 `app` 안에 `port/in`, `port/out`를 두는 방향으로 전환한다.
...
다음은 concrete class로 둘 수 있다.

- feature 내부 private orchestration helper
- 하나의 유스케이스 안에서만 쓰는 조립/검증 helper
- domain policy를 보조하는 내부 계산 서비스

즉, `서비스 = 무조건 interface + 구현체` 규칙은 채택하지 않는다.
...
단, pilot 단계에서는 기존 운영 중 서비스명을 일괄 변경하지 않는다.
즉 우선순위는 이름 변경보다 의존 방향 정리이며, 이미 널리 쓰이는 서비스는 아래처럼 기존 이름을 유지한 채 포트를 구현할 수 있다.
...
따라서 다음처럼 use case 관점으로 포트를 나눈다.
...
### 6. 전환 순서는 외부 경계 -> persistence pilot -> inbound 유스케이스 명시화 순으로 진행한다

우선순위:

1. 결제 / 알림 / 세션 저장소 같은 외부 경계 추출
2. `customer auth + guest claim` 도메인을 persistence pilot으로 전환
3. controller / batch / scheduler가 `UseCase` 인터페이스를 호출하도록 정리
4. order / booking / pass / product 도메인으로 확장

이 순서를 택하는 이유:

- 외부 경계는 교체 가능성이 높고 경계가 명확하다.
...
- **payment**: `RefundExecutionService`/`RefundRetryService`/`RefundPort`를 `app.booking`→`app.payment`로 이동, 주문과 예약 양쪽의 공용 환불 경계 확립
...
- **admin query**: `AdminBookingQueryService`/`AdminOrderQueryService` 도입 — 컨트롤러에서 infra 직접 의존 제거, User batch fetch 포함
