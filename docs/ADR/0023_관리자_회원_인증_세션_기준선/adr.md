...
- `users.version` 낙관적 락을 최종 stale update 방어선으로 사용한다. 비밀번호·휴대폰 확인과 같은 회원 변경은 행 잠금으로 직렬화한다. 고객 이메일 로그인 퍼사드는 기존 트랜잭션 참여를 금지하고 비관리 스냅샷 조회와 최초 BCrypt를 DB 트랜잭션 밖에서 수행한다. 성공 후보만 별도 트랜잭션 빈이 ID로 행을 잠근 뒤 활성 상태와 로컬 비밀번호 존재 여부를 확인하며, 스냅샷 이후 해시가 바뀐 경우에만 최신 해시로 BCrypt를 다시 수행한다. 강도를 높인 해시와 최근 로그인 시각은 이 잠금 트랜잭션에서 함께 저장해 실패 요청과 일반 BCrypt 비용이 회원 행 잠금 시간을 늘리지 않게 한다. 기존 소셜 로그인은 최근 로그인 시각을 갱신하므로 행 잠금 조회를 사용한다.
