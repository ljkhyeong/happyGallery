...
- 관리자 비밀번호도 비관리 projection으로 먼저 확인하고 성공 후보만 ID로 행을 잠근다. 잠금 뒤 해시가 사전 확인 때와 같으면 이미 끝난 BCrypt 결과를 재사용하고, 사이에 비밀번호가 바뀐 경우에만 최신 해시로 다시 확인한다.
...
- 로그인 성공·실패와 MFA 요구·실패·활성화·비활성화·복구 코드 사용은 `admin_auth_history`에 기록한다. 과거 `LOGIN_BLOCKED` 값은 기존 이력 역직렬화 호환을 위해 enum에만 유지한다. 입력 사용자명과 challenge 원문은 저장하지 않고 HMAC만 남기며, 이력은 개인정보 보존 배치에서 180일 뒤 삭제한다.
...
- `users.version` 낙관적 락을 최종 stale update 방어선으로 사용한다. 비밀번호·휴대폰 확인과 같은 회원 변경은 행 잠금으로 직렬화한다. 고객 이메일 로그인 퍼사드는 기존 트랜잭션 참여를 금지하고 비관리 스냅샷 조회와 최초 BCrypt를 DB 트랜잭션 밖에서 수행한다. 성공 후보만 별도 트랜잭션 빈이 ID로 행을 잠근 뒤 활성 상태와 로컬 비밀번호 존재 여부를 확인하며, 스냅샷 이후 해시가 바뀐 경우에만 최신 해시로 BCrypt를 다시 수행한다. 해시 승격과 최근 로그인 시각은 이 잠금 트랜잭션에서 함께 저장해 실패 요청과 일반 BCrypt 비용이 회원 행 잠금 시간을 늘리지 않게 한다. 기존 소셜 로그인은 최근 로그인 시각을 갱신하므로 행 잠금 조회를 사용한다.
- 회원 프런트는 로그인·로그아웃·회원 ID 변경과 활성 세션의 `401 UNAUTHORIZED`를 계정 세대 경계로 취급한다. 앱 최초 진입에서 확인한 비회원 `/me` 401은 계정 전환이 아니므로 세대를 올리거나 공개 화면 입력을 초기화하지 않는다. 각 경계는 `localStorage`의 `hg_customer_session_boundary`에 새 고유 epoch와 관찰한 회원 ID를 함께 기록해 같은 브라우저의 다른 탭에도 알린다. 이 값은 인증 근거가 아니라 변경 감지 신호이며 실제 회원은 `/me`로 다시 확인한다.
...
- 소셜 재인증·계정 연결처럼 브라우저 왕복이 여러 번 이어지는 작업은 `sessionStorage` continuation에 시작 회원 ID를 함께 저장한다. callback의 현재 회원이 다르면 후속 연결·해제와 redirect를 중단하고, 재인증 뒤 두 번째 OAuth 왕복을 시작할 때도 같은 소유자 ID를 다시 기록한다.
