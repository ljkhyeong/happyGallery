...
- `smartstore_stock_mappings`, `smartstore_stock_syncs`
  ...
  - V165의 `smartstore_stock_syncs.generation`은 기존 행에 `legacy`를 채우고 새 행은 Java UUID로 생성한다. 상품별 기존 행과 재등록 행을 구분하며, 같은 행의 재고 요청 버전이 증가할 때는 유지한다. 삭제·재등록 전 응답을 새 요청의 성공·실패로 기록하지 않는다. 늦은 외부 쓰기를 보정하기 위해 현재 전송 중·완료 요청은 최신 수량을 다시 요청하며, 기존 대기·최종 실패 정책은 유지한다.
