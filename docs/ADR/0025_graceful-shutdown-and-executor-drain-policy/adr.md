...
운영 중 애플리케이션 종료가 즉시 강제되면 다음 문제가 생길 수 있다.

- 이미 큐에 들어간 알림 작업이 중간에 유실된다.
- 종료 직전 진행 중이던 비동기 작업의 로그/MDC 문맥이 끊긴다.
- 외부 PG timeout 감시용 executor thread가 정리되지 않은 채 종료 순서가 불명확해진다.

반대로 종료 대기를 과도하게 길게 잡으면 deploy/rollback 시간이 늘어나고,
종료되지 않는 작업 때문에 인스턴스 교체가 지연될 수 있다.

따라서 "무조건 오래 기다린다"가 아니라, 애플리케이션과 개별 executor마다
명시적인 종료 대기 정책을 문서화할 필요가 있다.
...
이 기준은 "대부분의 짧은 비동기 후처리 작업은 마무리할 기회를 주되,
배포 파이프라인이 과도하게 멈추지 않도록 상한을 둔다"는 운영 의도다.
...
- 현재 알림 작업은 장시간 CPU 작업이 아니라 비교적 짧은 외부 호출/후처리 성격이므로
...
이 executor는 알림 executor와 다르게 "진행 중 작업 보존"보다 "보호용 thread 정리"가 우선이다.
...
동일한 종료 정책을 모든 executor에 일괄 적용하지 않는다.
종료 시 보호해야 하는 자산이 무엇인지에 따라 값을 다르게 둔다.
...
- `docs/ADR/0023_admin-auth-and-runtime-operations-baseline/adr.md`
- `app/src/main/resources/application.yml`
- `app/src/main/java/com/personal/happygallery/config/AsyncConfig.java`
- `infra/src/main/java/com/personal/happygallery/infra/payment/CircuitBreakerPaymentProvider.java`
