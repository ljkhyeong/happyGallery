...
통합 테스트는 단순한 컨트롤러 호출 검증이 아니라,
Flyway 마이그레이션, JPA 매핑, Redis 세션/레이트리밋, 필터 체인까지 포함한 실제 애플리케이션 경계를 검증해야 한다.

기존에는 `local` 프로필 혼용, 클래스별 프로퍼티 override, `MockMvc` 필터 설정 차이 때문에
테스트 컨텍스트가 불필요하게 분리되고 실행 비용이 커지는 문제가 있었다.

따라서 통합 테스트는 운영과 충분히 비슷한 인프라를 사용하되,
공유 가능한 프로파일과 컨텍스트 기준을 문서로 고정할 필요가 있다.
...
- 결과적으로 Flyway, JPA, Spring Session, Redis 기반 rate limit 경로를 실제와 가까운 방식으로 검증한다.
...
- 대신 `MockMvcBuilders.webAppContextSetup(...).addFilters(...)` 패턴으로 필요한 필터만 붙인다.
...
- `docs/Idea/0014_test-context-sharing-and-profile-separation/idea.md`
