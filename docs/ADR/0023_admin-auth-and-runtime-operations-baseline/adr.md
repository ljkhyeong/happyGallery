...
### 5. 비밀번호 저장과 에러 처리 구현은 공통 계층에서 유지한다
...
- 관리자 인증과 런타임 정책을 별도 ADR에서 관리할 수 있다.
