...
- 운영 캘린더 변경은 클래스 ID만 오름차순으로 조회한 뒤 해당 행을 기존 `IN (...) FOR UPDATE` 쿼리로 잠근다. 첫 조회에서 엔티티를 적재하지 않아 중복 로딩과 잠금 전 상태의 1차 캐시 저장을 피한다. 전체 범위를 한 번에 잠그는 쿼리는 신규 클래스 등록까지 대기시킬 수 있으므로 사용하지 않는다.
...
- 공공데이터포털의 한국천문연구원 특일 정보를 `public_holiday_snapshot`에 연도별로 저장하고 예약 판정에서 우선 사용한다.
  매일 04:20(서울)에 현재 연도와 다음 연도를 정상 응답일 때만 원자적으로 교체하고, 조회 실패 때는 마지막 정상 스냅샷을 유지한다.
