...
- 주문 토큰에 UNIQUE INDEX 추가 (V17 마이그레이션). 휴대폰 복구 토큰의 1:N 공유를 도입한 V109·V110에서 이 제약은 제거한다.
...
- 복구 POST 응답의 기존 배열은 주문·예약별 최신 100건으로 제한한다. 전체 이력은 같은 토큰을 사용하는 주문·예약 커서 API의 `{content,nextCursor,hasMore}` 응답으로 이어서 조회한다.
- V109·V110부터 복구 토큰 하나를 같은 비회원의 여러 주문·예약이 공유하므로 `access_token`은 UNIQUE가 아니다. 주문과 예약 모두 `(access_token, created_at DESC, id DESC)` 비유일 인덱스로 복구 이력을 조회한다.
