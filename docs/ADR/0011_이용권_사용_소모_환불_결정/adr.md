...
숨은 역순 경로를 허용하지 않는다. 예약 변경도 기존·신규 슬롯의 클래스를 PK 순으로 먼저 잠그므로,
여러 8회권 환불과 교차 클래스 예약 변경이 경합해도 클래스 락 순서는 같다. 이 순서는 서로 다른 클래스에서 같은 8회권을 동시에 쓰는 경우와
예약·취소·전체 환불·만료 경합을 동일하게 직렬화한다.
