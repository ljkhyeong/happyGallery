...
다른 개인 프로젝트를 같은 k3s에서 운영하는 안은 [프로젝트별 구성과 격리 검토](../../../deploy/laptop/multi-project.md)에 정리했다. 현재는 권고 단계이며, 이 ADR이나 기존 namespace·컨테이너 제한만으로 프로젝트 전체 자원·통신·디스크 격리가 완료됐다고 판단하지 않는다.
