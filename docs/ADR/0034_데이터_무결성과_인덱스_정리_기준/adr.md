...
- 휴대폰 번호 인증 뒤 발급한 복구 토큰은 같은 Guest의 여러 주문·예약에 1:N으로 공유한다.
...
- 활성 예약 UNIQUE 위반은 `409 DUPLICATE_BOOKING`, 회원 전화번호와 소셜 연결 UNIQUE 위반은
  각 소유권 충돌 코드로 변환한다. 슬롯 시작 시각 중복처럼 계약상 의미가 확정된 제약만 명시적으로
  입력 오류로 번역한다. 이름을 알 수 없거나 매핑하지 않은 DB 무결성 위반은 클라이언트 입력으로
  추측하지 않고 throwable과 제약 이름을 기록한 `500 INTERNAL_ERROR`로 처리한다.
