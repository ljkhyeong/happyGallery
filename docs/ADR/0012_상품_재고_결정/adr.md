...
옵션 조합의 기본가와 추가금 검증은 `ProductVariant` 생성·수정에서 한 번 수행한다.
서비스는 검증된 조합 가격에 직접입력 옵션 추가금을 합쳐 전체 금액 상한을 확인한다.
입력 record의 `List.copyOf`가 거부하는 null 원소는 서비스에서 다시 검사하지 않으며,
옵션 식별자·중복·필수 선택 조건은 계속 검사한다.

상품 이력과 스마트스토어 연결 계획처럼 존재 여부만 필요한 조회는 Spring Data의 `existsById`를 사용한다.
재고·연결 설정을 변경할 때의 엔티티 조회와 잠금은 유지한다.
...
- 단건 추가는 기존 행의 상품·옵션·문구로 키를 다시 계산해 같은 항목 중 가장 작은 ID에 수량을 더한다. 한 항목을 찾기 위한 Map은 만들지 않고, 여러 항목을 찾는 병합에서만 키별 Map을 사용한다.
- 병합 요청의 각 수량과 같은 항목의 합계는 모두 1~99개로 제한한다. 매번 합계를 검사하므로 두 수의 덧셈은 최대 198이며 별도 정수 오버플로 예외 처리는 하지 않는다.
