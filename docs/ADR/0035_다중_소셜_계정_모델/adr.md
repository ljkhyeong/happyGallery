...
19. `CustomerUserResponse.email`은 검증된 기준 이메일이 없을 때 `null`이다. 신규 Naver 전용 회원은 최근 본인 확인과 별도 메일함 인증을 거쳐 기준 이메일을 한 번 등록할 수 있다. 검증된 휴대폰도 있으면 기존 SMS 비밀번호 재설정으로 최초 로컬 비밀번호를 설정한다.
