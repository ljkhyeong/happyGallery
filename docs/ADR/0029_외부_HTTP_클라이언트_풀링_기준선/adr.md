...
### 5. API 응답의 리다이렉트를 자동으로 따라가지 않는다

- `PooledHttpClientFactory`에서 자동 리다이렉트를 끈다. 기본 동작에서는 `X-Secret-Key` 같은 사용자 정의 인증 헤더가 다른 주소로 전달되고, 307·308 응답에서는 POST 본문도 재전송된다.
- HTTP 클라이언트는 원래 3xx 응답을 호출부에 전달한다. API 주소가 바뀌면 제공자의 공식 주소를 확인해 설정을 수정한다. 브라우저의 소셜 로그인 이동에는 적용하지 않는다.
- 우체국·네이버 메일의 중복 설정을 공통 설정으로 통합하고, 사용처가 없어진 클라이언트 설정 콜백을 제거한다.
- `PooledHttpClientFactoryTest`는 서로 다른 포트의 로컬 서버 두 대로 GET 302와 POST 307·308을 확인한다. 이동 대상에 요청·인증 헤더·본문이 전달되지 않아야 한다.
...
- 제공자가 API 주소를 리다이렉트하면 공식 주소를 설정에 반영해야 한다.
