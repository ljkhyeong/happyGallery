...
- Prometheus는 결제 `RECONCILIATION_REQUIRED` DB backlog, 환불 `FAILED`·`RECONCILIATION_REQUIRED`, 상태별 처리 기준 시각을 5분 넘긴 자동 복구 backlog, 알림 outbox `FAILED`, 선점 후 2분 넘은 `PROCESSING`·`DELIVERY_CHECKING`, 재시도 또는 결과 조회 예정 시각을 1분 넘긴 `PENDING`·`DELIVERY_PENDING`, 1분 넘게 중단된 스냅샷 갱신을 각각 알린다. future `next_attempt_at`의 age는 0이므로 정상 백오프와 결과 조회 대기는 정체로 보지 않는다. Grafana system dashboard도 같은 상태별 건수·처리 기준 경과 시간·스냅샷 갱신 나이를 표시한다.
