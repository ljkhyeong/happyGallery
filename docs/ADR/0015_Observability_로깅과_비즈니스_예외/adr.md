...
- `OperationalBacklogMetrics`는 환불과 알림 outbox를 상태별 `COUNT`와 처리 기준 시각의 `MIN` 그룹 쿼리로 15초마다 스냅샷한다. 스마트스토어 주문 처리의 `RESULT_UNKNOWN`과 5분 넘게 끝나지 않은 `REQUESTED`도 같은 방식으로 미대사 건수와 가장 오래된 요청 경과 시간을 집계한다. scrape마다 DB를 조회하지 않으며, 조회 실패 때 마지막 정상 값을 유지하고 갱신 실패 횟수와 마지막 정상 갱신 후 경과 시간을 별도 지표로 노출한다.
...
- Prometheus는 결제 `RECONCILIATION_REQUIRED` DB backlog, 스마트스토어 주문 처리 미대사 backlog, 환불 `FAILED`·`RECONCILIATION_REQUIRED`, 상태별 처리 기준 시각을 5분 넘긴 자동 복구 backlog, 알림 outbox `FAILED`, 선점 후 2분 넘은 `PROCESSING`·`DELIVERY_CHECKING`, 재시도 또는 결과 조회 예정 시각을 1분 넘긴 `PENDING`·`DELIVERY_PENDING`, 1분 넘게 중단된 스냅샷 갱신을 각각 알린다. future `next_attempt_at`의 age는 0이므로 정상 백오프와 결과 조회 대기는 정체로 보지 않는다. Grafana system dashboard도 같은 상태별 건수·처리 기준 경과 시간·스냅샷 갱신 나이를 표시한다.
