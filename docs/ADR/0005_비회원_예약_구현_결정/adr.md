...
- 휴대폰 번호 인증으로 발급한 복구 토큰은 같은 비회원의 여러 예약에 공유될 수 있다. 따라서 토큰 자체를 UNIQUE로 제한하지 않고 예약 ID와 함께 소유권을 확인한다.
