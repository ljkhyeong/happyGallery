...
`PaymentProvider.refund()` 외부 호출 경계에 `CircuitBreaker + TimeLimiter`를 먼저 적용하면,
실 PG 연동 전에도 장애 전파를 줄이고 운영 전환 시 설정 튜닝 중심으로 대응할 수 있다.
...
- 실제 PG가 아닌 Fake 어댑터 환경에서도 보호 경계를 선반영해 운영 전환 시 구조 변경보다 설정 튜닝에 집중할 수 있게 됐다.

---

## 결론

이 실험은 유효했고, 결제 환불 경계 보호는 정식 채택 상태로 본다.
...
- `docs/Idea/0002_bulkhead-resilience4j-consideration/idea.md`
- `plan.md`
