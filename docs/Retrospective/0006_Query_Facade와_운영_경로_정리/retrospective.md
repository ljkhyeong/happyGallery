...
`main` 히스토리에는 기능 구현 직후 컨트롤러 조합 로직이나 직접 의존을 줄이는 리팩터가 반복된다.
...
- 컨트롤러가 가격 조회, 상태 필터, 응답 조합, 소유권 판정을 직접 들고 있는 경우가 남는다.
- 관리자 조회와 회원 조회는 상태 변경 유스케이스보다 정리 순서가 뒤로 밀리기 쉽다.
- 알림, 환불, 감사 로그는 기능 자체는 맞지만, 저장, 재시도, 운영 조회 경로가 기능별로 흩어져 있다.
...
현재:
- 컨트롤러가 조회 조합 규칙을 직접 알고 있다.

정리 방향:
- `Admin*QueryService`, `Me*QueryService` 같은 조회 조합 서비스를 두고 컨트롤러는 HTTP 매핑과 요청 검증에 더 가깝게 유지한다.
...
현재:
- 환불, 알림 로그, 감사 기록이 도메인별 서비스에 흩어져 있다.

정리 방향:
- 공용 환불 경계, 알림 로그 저장 경계, 운영 조회 모델을 정리해
  주문, 예약, 배치가 같은 방식으로 호출하도록 맞춘다.

---

## 현재 결론

히스토리상 “기능 구현 후 조회/컨트롤러 정리”는 반복되는 패턴이다.  
