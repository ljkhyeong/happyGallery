...
`main` 히스토리 후반으로 갈수록 local 전용 seed, dev hook, 테스트 지원 API가 점점 늘어났다.

대표 구간:
- `e009c5d` 프로필 분리
- `fc105cf` admin 인증 필터
- member/guest E2E 확장 구간의 local helper 반영
- 관측성과 운영 보강 구간의 dev support 증가
...
- local/dev 편의 기능은 처음에는 빠르게 유용하지만,
  profile 경계가 약하면 운영 기본값과 섞이기 쉽다.
- local-only controller/service가 feature별로 흩어지면
  어떤 것이 운영 코드이고 어떤 것이 지원 도구인지 읽기 어려워진다.
- 테스트와 E2E가 커질수록 dev support API는 늘어날 가능성이 높다.
...
As-Is:
- local seed, fail hook, debug lookup이 feature별로 흩어진다.
...
문제는 “존재 여부”가 아니라 “기본 설정과 운영 코드에 얼마나 섞이느냐”이므로,
