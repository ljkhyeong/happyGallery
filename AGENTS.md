...
- `ps`, `lsof`, `curl`, `git status`, `git log`, `ls`, `rg`, `sed` 같은 로컬 조회/확인 명령은 저위험으로 보고 사용자에게 사전 확인 메시지를 보내지 않고 바로 실행한다.
- 빌드, 테스트, 헬스 체크, 프로세스/포트 확인처럼 검증 목적이 분명한 명령도 사용자에게 "실행해도 되는지"를 다시 묻지 않는다. 샌드박스 때문에 권한 상승이 필요하면 별도 채팅 확인 없이 바로 승인 요청을 띄운다.
