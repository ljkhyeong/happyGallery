 ...
 * 실제 도메인 유틸 구현 후 이 패턴을 그대로 적용한다(PLAN.md 4.1).
 *
...
class TimeBoundaryPolicyTest {
    ...
    void dMinus1_oneMinuteBefore_refundAllowed() {
        ...
        ZonedDateTime deadline = experienceDate.atStartOfDay(SEOUL); // 2026-03-01T00:00+09:00

        Clock before = Clock.fixed(deadline.minusMinutes(1).toInstant(), SEOUL);
    ...
    void dMinus1_atMidnight_refundNotAllowed() {
        LocalDate experienceDate = LocalDate.of(2026, 3, 1);
        ZonedDateTime deadline = experienceDate.atStartOfDay(SEOUL);

        Clock atDeadline = Clock.fixed(deadline.toInstant(), SEOUL);
    ...
    void sameDay_61MinutesBefore_changeAllowed() {
        ZonedDateTime slotStart = ZonedDateTime.of(2026, 3, 1, 14, 0, 0, 0, SEOUL);
        ZonedDateTime changeDeadline = slotStart.minusHours(1); // 13:00

        Clock before = Clock.fixed(slotStart.minusMinutes(61).toInstant(), SEOUL);
    ...
    void sameDay_atChangeDeadline_changeNotAllowed() {
        ...
        Clock atDeadline = Clock.fixed(changeDeadline.toInstant(), SEOUL);
