...
- 주문제작 상품은 고정 사양과 1~180일 예상 제작 기간을 필수로 관리한다. 관리자는 필수·선택 선택형 옵션, 각인 같은 직접입력 옵션과 선택 조합별 추가 금액·재고·판매 여부를 등록한다. 고객은 서로 다른 조합과 문구를 여러 품목으로 담을 수 있고, 서버는 결제 준비 시 SKU 재고와 `기본가 + 옵션 추가금`을 다시 확정해 상품 유형·옵션·문구·사양·관리 방법·제작 기간과 함께 주문 항목에 고정한다.
