...
결제창 취소·실패 화면은 같은 고객 세션의 구매 화면으로 돌아가는 버튼을 제공한다. 상품·주문서·장바구니·8회권은 원래 경로로, 예약은 선택한 클래스의 예약 화면으로 돌아가 최신 가능 시간을 다시 고른다. 복귀는 화면 이동만 하며 결제를 자동 재요청하지 않는다. 저장 정보가 없거나 고객이 바뀌었거나 외부 주소이면 홈·상품 목록 링크만 제공한다.
