...
- 공개 사용자 흐름
  ...
  - 상품 상세에서 회원 주문 생성
  - legacy 비회원 주문 fallback (`/orders/new`, 상품/수량 prefill 지원)
  - 비회원 예약 조회/변경/취소 (`/guest/bookings`)
  - 비회원 주문 조회 (`/guest/orders`)
...
- 현재 smoke 범위는 guest/admin 1~5와 member storefront/claim 6~8이며, `P8-7`은 회원 예약 상세/변경/취소, `P8-8`은 guest claim까지 포함한다.
...
- canonical guest 조회 경로는 `/guest/orders`, `/guest/bookings`이고, `/orders/detail`, `/bookings/manage`는 redirect alias다.
...
- 회원 UI는 `/my`, `/my/orders/:id`, `/my/bookings/:id`를 사용하고, 백엔드는 `/api/v1/me/**`로 동작한다.
...
- 상품 상세에서 guest 주문으로 넘길 때는 `/orders/new`가 `productId`, `qty` query를 받아 초기 주문 항목을 채운다.
