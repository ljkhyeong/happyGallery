...
- 스마트스토어 주문 상세의 수령인·연락처·배송지는 암호문으로 저장하고 관리자 단건 화면에서만 복호화한다. 관리자는 발주 확인, 발송·지연, 취소·반품 승인/거부, 교환품 재배송을 네이버 API로 처리한다. 최근 상품 문의를 조회·답변하고, 매시 50분 최근 7일 정산을 채널 주문의 정산 예정액과 대조해 불일치만 오늘 할 일에 표시한다. 외부 처리 성공 직후 로컬 상태를 추측해 바꾸지 않고 다음 주문 변경 피드에서 확정 상태를 수집한다.
