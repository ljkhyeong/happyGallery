...
- 스마트스토어 주문 상세의 수령인·연락처·배송지는 암호문으로 저장하고 관리자 단건 화면에서만 복호화한다. 주문 목록은 확인 사유와 커서로 조회하며, 매핑 누락·새 상태 주문은 관리자가 내부 상품·옵션 조합과 남은 수량 차감·전체 복원·현재 차감 유지를 직접 결정한다. 재고 부족 주문은 연결된 상품·옵션의 재고 조정 화면으로 바로 이동한다. 관리자는 발주·발송을 최대 30건씩 부분 성공 결과와 함께 처리하고, 단건 취소·반품 승인/거부·반품 보류/해제·판매자 반품 요청, 교환 수거·보류·거절·재배송을 네이버 API로 처리한다. 외부 주문 요청과 수동 재고 결정은 처리자·요청 내용과 성공·미전송·거절·결과 미확정 상태를 주문별 최근 이력에 남긴다. 결과 미확정 또는 5분 넘게 끝나지 않은 요청은 오늘 할 일에 표시하고, 관리자가 네이버 현재 상태나 판매자센터를 확인한 뒤 반영 여부와 근거를 기록한다. 클레임 첨부 이미지를 상세에서 확인하고 택배사 코드는 자주 쓰는 목록에서 선택하거나 직접 입력할 수 있다. 상품 문의와 주문·배송 고객 문의를 함께 조회·답변하며 네이버 상품 문의 답변 템플릿을 적용할 수 있다. 정산 지급일 커서로 장애 기간의 누락 날짜를 순차 복구하고, 별도 회계 화면에서 31일 이내 일별 정산·수수료·전월까지의 부가세 자료를 CSV로 내려받는다. 상품 가격·판매 상태·옵션 가격은 채널 차이를 먼저 확인한 뒤 명시적으로 반영하며, 외부 처리 성공 직후 로컬 상태를 추측해 바꾸지 않고 다음 주문 변경 피드에서 확정 상태를 수집한다.
...
목표 운영 환경은 월 2~3만 원 내외의 단일 클라우드 VM과 외부 백업 저장소다. 한국 이용자 기준으로 AWS Lightsail 서울을 우선 검토하며, 기존 일본 8GB 견적은 구매 보류다. 기존 k3s 구성의 2GB·4GB 운영 가능 여부는 자원 조정과 부하 검증으로 판단한다. 서버 구매 후보, 비용 계산과 실제 개통 순서는 [클라우드 운영 준비](deploy/cloud/README.md)를 따른다. 공급자·사양·예산 변경과 구매는 아직 확정하지 않았다.
