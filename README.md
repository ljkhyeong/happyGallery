...
회원 주문·예약·8회권은 전체 이력을 검색·정렬한 뒤 20건씩 조회한다. 검색 결과가 없어도 조건을 바꿀 수 있으며, 상품 검색 조건은 URL에 남아 뒤로 가기와 새로고침 후에도 유지된다.

회원·비회원은 배송 준비 전 주문 상세에서 배송지를 수정할 수 있습니다.

관리자는 재고 조정 화면에서 최소 보유 수량을 설정하고 오늘 할 일에서 품절·재고 부족 항목을 확인합니다.

회원은 품절 상품·옵션의 재입고 알림을 신청하고 내 정보에서 확인·해지할 수 있습니다.
단체 수업 문의는 공개 화면에서 접수하고, 회원은 내 정보에서 상태를 확인하며 관리자는 외부 문의 등록과 상담 이력을 관리합니다.
...
- 주문·예약·8회권은 공통 결제 API(`prepare` → `confirm`)를 사용한다. 고객은 결제 결과와 영수증을 다시 조회하고, 비회원은 휴대폰 인증으로 결제 조회 정보를 복구할 수 있다. 승인·실패 처리는 [결제 ADR](docs/ADR/0033_결제_confirm_트랜잭션과_보상_경계/adr.md), 웹훅·정산은 [기준 스펙](docs/PRD/0001_기준_스펙/spec.md), 요청·응답은 [API 계약](docs/PRD/0004_API_계약/spec.md)을 따른다.
...
- 상품 상세 바로구매·장바구니·주문서는 같은 쿠폰·적립금 선택과 금액 요약을 사용한다. 선택한 옵션·수량의 상품 합계에서 혜택을 차감하고 배송비를 더한 결제 예정 금액을 표시하며, 실제 할인액과 결제액은 서버의 기존 결제 준비 단계에서 확정한다.
- 회원 장바구니는 전체·개별 선택 구매를 지원한다. 선택 상품만 금액·쿠폰·적립금·주문제작 동의에 반영하며, 미선택 상품은 결제 후 장바구니에 남는다. 각인 문구가 달라도 같은 상품·옵션의 선택 수량을 합산해 재고를 확인하므로, 전체를 살 재고가 부족해도 일부만 선택해 구매할 수 있다. 세부 계약은 [API 계약](docs/PRD/0004_API_계약/spec.md)을 따른다.
- 상품 상세에서 각인 문구가 달라도 동일 옵션 조합의 수량은 합산한다. 옵션 추가·수량 변경은 현재 상품 응답의 조합 재고와 조합별 99개 한도를 함께 적용하고, 남은 추가 가능 수량을 표시한다. 실제 결제 준비 단계의 최신 재고 검증은 서버가 계속 수행한다.
- 비회원 장바구니도 기존에 담긴 수량을 포함해 재고·조합별 99개 한도를 적용한다. 재고가 줄면 수량 조정을 안내하고 감소·삭제는 허용한다. 여러 옵션 담기는 한 번에 저장하며 회원은 기존 다건 병합 API의 멱등 처리를 재사용한다.
- 회원도 재고 감소·판매 중지·옵션 변경과 무관하게 본인 장바구니 수량을 줄이거나 삭제할 수 있다. 추가·증가에만 현재 재고와 판매 옵션을 확인한다.
- 상품 상세의 선택 행은 옵션·문구·수량만 보관하고 현재 상품 응답으로 단가·표시명·합계를 계산한다. 비회원 장바구니·주문서도 같은 계산을 사용하며, 선택한 옵션이 사라지면 삭제 후 재선택을 안내한다. 결제 후 구매 당시 가격 보존은 바꾸지 않는다.
- 각인 문구의 구분문자는 그대로 보존한다. 프런트 항목 키는 JSON으로, 서버 항목 키·병합 요청 해시는 Base64로 인코딩한 입력 값으로 구분한다. V162는 기존 장바구니의 ID·수량을 유지하고 키만 전환한다. 배포 전환 주의사항은 [상품·재고 ADR](docs/ADR/0012_상품_재고_결정/adr.md)에 기록한다.
- V163부터 각인 키는 표시 순서가 아닌 옵션 키 순으로 계산한다. 기존 중복 행은 ID·수량을 보존하고 이후 추가분은 같은 각인의 가장 오래된 행에 합산한다. 전환 전후 백엔드를 혼용하지 않는다.
- 비회원 주문서도 품목별 선택 옵션과 각인 문구·직접입력 추가금을 결제 전에 표시한다. 고객 장바구니·주문 상세와 같은 표시 컴포넌트를 사용하되, 주문 완료 후에는 저장된 구매 당시 옵션 정보를 표시한다.
- 옵션 없는 주문제작 상품도 기본 조합 추가금을 상세와 주문서에 동일하게 표시한다. 옵션 없는 상품의 수량도 현재 재고와 99개 한도를 적용하며, 주문서는 판매 중지·옵션 변경·합산 재고 초과 항목이 있으면 결제를 막고 수량 조정·삭제·재선택을 안내한다.
- 주문서 초안은 주문 화면마다 별도 번호로 저장하고 같은 고객 세션에서만 복원한다. 옵션 없는 바로구매·수동 입력에도 번호를 붙이며, 수량 변경·상품 추가·삭제와 전체 삭제 결과를 새로고침·결제 실패 복귀 후에도 유지한다. 인증코드는 저장하지 않는다. 초안 누락·세션 변경 시 기본 주문으로 대체하지 않으며, 변경 내용의 저장 실패는 현재 입력을 유지한 채 재저장을 안내하고 결제를 막는다.
- 관리자가 선택형 옵션 그룹·값을 삭제해도 남아 있는 옵션을 새로 고를 수 있다. 비회원 장바구니의 추가·증가는 최신 상품의 옵션 유효성도 확인하므로 삭제된 각인이나 새 필수 입력이 빠진 항목은 저장하지 않는다. 여러 품목 중 하나라도 유효하지 않으면 전체 담기가 실패하며 기존 항목의 감소·삭제는 계속 허용한다.
- 쿠폰·적립금 전액 결제와 8회권 사용 예약도 공통 결제 결과 화면에서 확정한다. 응답이 끊겨도 같은 결제 번호로 결과를 조회·재확인하며, 미확인 0원 결제가 있으면 새 결제 준비 대신 해당 결과 화면으로 연결한다. 복구 정보를 브라우저에 저장하지 못하면 승인하지 않고 준비된 결제를 종료한다.
- 결제 완료 뒤에는 승인 요청을 지우고 URL에 조회용 `completedOrderId`만 남긴다. 완료 화면 새로고침은 기존 소유권 확인 상태 조회 API만 사용해 내역·영수증 링크를 복원하며, 조회 실패를 결제 실패로 표시하거나 승인을 재요청하지 않는다.
- 비회원 주문·예약은 결제 준비 성공 시 사용한 인증코드를 초기화한다. 결제창 취소나 인증 실패 뒤에는 선택한 상품·시간과 이름을 유지한 채 새 인증코드를 받을 수 있다. 재고 부족 등 준비 단계의 업무 오류는 인증 입력을 유지하고, 재발송하면 이전 코드로 결제를 진행할 수 없게 한다.
- 이메일·소셜 최초 가입과 비회원 주문·예약 결제는 `GET /api/v1/policies/current`로 조회한 이용약관·개인정보처리방침 버전에 동의해야 한다. 서버는 현재 버전을 다시 검증하고 회원 또는 결제 시도에 동의 유형·목적·서버 수락 시각을 추가 이력으로 보존한다. 프론트는 기존 약관·방침 본문을 유지하고 개정 시 새 버전을 추가해 동의 당시 본문을 계속 제공한다.
- 마이페이지 주문·예약 상세와 8회권 목록에서 결제 영수증을 다시 열 수 있다. 현재 거래 소유권을 확인한 뒤 기존 결제 이력에서 URL을 조회하며 8회권은 목록 단위로 일괄 조회한다. 0원·과거 미기록·영수증이 없는 결제에는 버튼을 표시하지 않는다.
...
- 환불은 요청 이력을 먼저 커밋한 뒤 PG를 호출한다. 실행 유실·일시 실패는 최초 멱등키로 복구하고, 결과 불명 상태는 PG 취소 내역과 대조해 환불 성공 여부를 확인한 뒤 재요청 여부를 결정한다.
...
- 관리자는 이미지·상품명·채널상품 번호가 보이는 네이버 상품 목록에서 스마트스토어 원상품을 선택하고, 주문제작 상품은 원상품 상세에 표시된 옵션 조합명·가격·재고를 보면서 각 내부 옵션 조합에 네이버 옵션 ID를 매핑한다. 주문 차감·환불 복구·수동 조정과 같은 재고 변경은 같은 트랜잭션에서 동기화 요청으로 합쳐지고, 배치가 최신 절대 수량만 스마트스토어에 반영한다. 기성품은 원상품 재고, 주문제작품은 모든 옵션 조합 재고를 한 상품 단위 요청으로 전송한다. 원상품 변경·해제 뒤 늦게 수집된 기존 주문은 주문 식별 전용 과거 연결로 내부 재고에 반영하고, 등록·변경·해제 처리자와 설정 스냅샷은 관리자 화면의 최근 이력에서 확인한다. 네이버 검수 반려 상품은 실시간 목록에서 사유·조치 방법을 확인하고 수정 뒤 복원 요청할 수 있으며, 상품 공지사항도 등록·수정·삭제하고 선택한 채널상품에 적용할 수 있다.
- 스마트스토어 주문 처리는 다음 기능을 제공한다. 개인정보 저장·외부 요청·정산 처리 규칙은 [주문 연동 ADR](docs/ADR/0048_스마트스토어_주문_운영_연동/adr.md), 재고 반영은 [재고 동기화 ADR](docs/ADR/0047_스마트스토어_재고_동기화/adr.md)을 따른다.
  - 주문 처리: 발주·발송·취소·반품·교환을 처리하고, 결과가 불명확한 요청은 오늘 할 일에서 네이버 반영 여부를 확인한다.
  - 재고 관리: 상품 연결이 없거나 새 상태인 주문의 재고 반영 방법을 정하고, 재고 부족 주문에서 해당 상품·옵션의 재고 조정 화면으로 이동한다.
  - 문의 답변: 상품·주문·배송 문의를 조회하고 네이버 답변 템플릿을 적용한다.
  - 정산 조회: 누락된 지급일의 정산을 다시 수집하고 정산·수수료·부가세 자료를 CSV로 내려받는다.
- 스마트스토어 상품 문의와 주문·배송 문의는 기존 답변을 불러와 수정·저장하거나 취소할 수 있으며, 저장 실패 시 초안을 유지한다. 문의 조회 실패 중에도 탭·조회 조건·재시도 버튼을 제공하고 이미 불러온 목록과 편집 중인 답변은 유지한다. 반품·교환 택배사 계약 목록은 주문 화면에서 실시간 조회한다. 목록의 계약번호는 주문 발송·수거용 택배사 코드와 달라 자동 대입하지 않는다.
- 스마트스토어 문의는 시작일·종료일을 선택해 50건씩 이전·다음 페이지로 조회한다. 초기 기간은 한국 날짜 기준 오늘을 포함한 30일이며, 상품 문의와 주문·배송 문의의 기간·미답변 필터·페이지는 탭별로 유지한다. 서버는 선택한 네이버 페이지만 읽고 전체 문의를 미리 수집하지 않는다.
- 스마트스토어 반품은 확인창에 표시된 미검수 수량을 검수한 뒤 복원하거나 복원 없이 종료한다. 확인 중 추가 반품이나 다른 관리자의 검수가 발생하면 이전 요청을 거절하고 최신 수량을 다시 확인한다. 검수 대상 버전이 필수이므로 서버와 관리자 화면을 함께 배포하고 이전 화면은 새로고침한다.
- 운영 알림은 NHN의 요청 접수 식별자를 저장한 뒤 최종 수신 결과를 별도 조회한다. 실제 성공이 확인된 건만 알림함의 `SENT`로 확정하고, 알림톡 최종 실패가 확인된 경우에만 SMS를 다음 채널로 요청한다.
- 알림 목록과 상단 알림함에는 주문 번호·구매 당시 상품명, 예약 클래스·현재 예약일, 재입고 상품·옵션을 함께 표시한다. 본인 원본을 조회할 수 없으면 관련 정보는 비운다.
- 배송·픽업이 끝난 주문은 회원 세션 또는 비회원 접근 토큰으로 품목·수량별 환불·교환 클레임을 접수한다. 관리자는 환불액과 반품 재고 복구 여부를 결정하고, 고객 반환액을 PG 환불과 적립금 복원으로 분해해 상품별 배분을 승인 시 고정한다. 교환품 재고는 승인 시 다시 차감하며 PG 환불은 기존 비동기 환불 재처리 절차를 사용하고 0원 PG 주문은 내부 혜택 복구만으로 종결한다.
...
- 고객은 클래스별 향후 1~30일 회차를 날짜별로 탐색하고 일반 예약은 해당 클래스의 남은 회차 정원까지 접수한다. 만석 회차에는 회원 또는 휴대폰을 인증한 비회원이 1회성 빈자리 알림을 신청하며, 좌석을 선점하지 않고 자리가 실제 예약 가능해진 순간 안내받아 선착순으로 예약한다. 회원은 예약 화면에서 사라진 회차의 대기 알림도 마이페이지에서 클래스명·일시와 함께 확인하고 취소할 수 있다. 인원만큼 슬롯 정원과 예약금·잔금을 반영하며, 예약금 환불 마감 전에는 1명 이상을 남겨 인원을 부분취소하고 줄어든 예약금만 환불받을 수 있다. 8회권 예약은 1명으로 제한한다. 운영자 사정으로 예약을 취소하면 고객 취소 마감과 무관하게 PG 예약금 환불 또는 유효한 8회권 크레딧 복구를 시작한다. 오프라인에서 받은 예약금, 이미 받은 현장 잔금과 만료 이용권 보상은 금액과 사유가 있는 영속 후속 작업으로 남겨 관리자가 완료 처리한다.
...
- 회원은 내 단체 문의에서 확정 전 일정·인원을 수정하거나 취소하고 본인 변경 이력을 확인한다. 관리자는 접수 번호·기간·문의 경로를 함께 검색하며 고객 취소 문의는 연락 예정 목록에서 제외된다.

- 주문 상세에서 구매 상품을 현재 가격·옵션·재고로 확인한 뒤 다시 담을 수 있다. 변경된 옵션이나 각인 문구는 상품 상세에서 다시 선택하고, 같은 요청의 재시도는 중복 추가하지 않는다.
...
상품 상세·주문서·장바구니·예약금·8회권 구매 화면에서 `카드·간편결제`, `네이버페이`, `카카오페이`를 선택한다. 네이버페이와 카카오페이는 토스 경유 자체창으로 열며 별도 PG 서버를 두지 않는다. 전용창 선택 시 토스 결제 약관 동의를 받은 뒤 prepare를 요청하고, 승인·부분취소·정산은 기존 토스 경로를 사용한다. 결제수단을 바꾸면 약관 동의를 다시 받는다. 운영 사용 전 토스 가맹점에서 두 간편결제를 사용할 수 있는지 확인하고 실제 결제·전체취소·부분취소를 검증해야 한다. [공식 자체창 연동 안내](https://docs.tosspayments.com/guides/v2/payment-window/integration-direct)

결제창 취소·실패 화면은 같은 고객 세션의 구매 화면으로 돌아가는 버튼을 제공한다. 복귀 전에 저장된 결제 ID로 `POST /api/v1/payments/{orderId}/abandon`을 호출해 승인 전 결제와 쿠폰·적립금 예약을 함께 종료한다. 승인 처리가 이미 시작됐으면 복귀하지 않고 현재 결제 상태를 표시한다. SDK가 현재 화면에서 오류를 반환해도 같은 종료를 시도한다. 브라우저 자체를 닫거나 종료 요청이 실패한 경우에는 기존 30분 만료 배치가 미시작 결제를 정리한다. 상품·주문서·장바구니·8회권은 원래 경로로, 예약은 선택한 클래스에서 최신 가능 시간을 다시 고르며 새 결제는 자동 요청하지 않는다. 저장 정보 없음·고객 변경·외부 주소는 홈·상품 목록으로 안내한다.
...
- 회원 주문 번호·구매 당시 상품명 검색, 목록의 상품명·옵션 표시
- 내 정보의 기본 배송지 저장·삭제와 주문서에서 불러오기
- 상품·클래스 상세의 찜 저장·해제와 내 정보의 종류별 목록
- 관리자 단체 문의의 다음 연락일과 오늘 할 일, 상품·옵션별 재입고 알림 대기 인원

정책과 API 범위는 [기준 스펙](docs/PRD/0001_기준_스펙/spec.md)과 [API 계약](docs/PRD/0004_API_계약/spec.md)을 따른다.
