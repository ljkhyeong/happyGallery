...
- 공개 사용자 흐름
  ...
  - 회원 주문/예약/8회권 전체 목록 (`/my/orders`, `/my/bookings`, `/my/passes`, 검색/상태 필터 포함)
...
- canonical guest 조회 경로는 `/guest/orders`, `/guest/bookings`이다.
...
- 회원 UI는 `/my`, `/my/orders`, `/my/orders/:id`, `/my/bookings`, `/my/bookings/:id`, `/my/passes`를 사용하고, 목록 페이지는 검색/상태 필터를 제공한다. 백엔드는 `/api/v1/me/**`로 동작한다.
