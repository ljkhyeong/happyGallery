...
- 관리자는 이미지·상품명·채널상품 번호가 보이는 네이버 상품 목록에서 스마트스토어 원상품을 선택하고, 주문제작 상품은 원상품 상세에 표시된 옵션 조합명·가격·재고를 보면서 각 내부 옵션 조합에 네이버 옵션 ID를 매핑한다. 주문 차감·환불 복구·수동 조정과 같은 재고 변경은 같은 트랜잭션에서 동기화 요청으로 합쳐지고, 배치가 최신 절대 수량만 스마트스토어에 반영한다. 기성품은 원상품 재고, 주문제작품은 모든 옵션 조합 재고를 한 상품 단위 요청으로 전송한다. 네이버 검수 반려 상품은 실시간 목록에서 사유·조치 방법을 확인하고 수정 뒤 복원 요청할 수 있으며, 상품 공지사항도 등록·수정·삭제하고 선택한 채널상품에 적용할 수 있다.
- 스마트스토어 주문 상세의 수령인·연락처·배송지는 암호문으로 저장하고 관리자 단건 화면에서만 복호화한다. 관리자는 발주·발송을 최대 30건씩 부분 성공 결과와 함께 처리하고, 단건 취소·반품 승인/거부·반품 보류/해제·판매자 반품 요청, 교환 수거·보류·거절·재배송을 네이버 API로 처리한다. 클레임 첨부 이미지를 상세에서 확인하고 택배사 코드는 자주 쓰는 목록에서 선택하거나 직접 입력할 수 있다. 상품 문의와 주문·배송 고객 문의를 함께 조회·답변하며 네이버 상품 문의 답변 템플릿을 적용할 수 있다. 정산 지급일 커서로 장애 기간의 누락 날짜를 순차 복구하고, 별도 회계 화면에서 31일 이내 일별 정산·수수료·전월까지의 부가세 자료를 CSV로 내려받는다. 상품 가격·판매 상태·옵션 가격은 채널 차이를 먼저 확인한 뒤 명시적으로 반영하며, 외부 처리 성공 직후 로컬 상태를 추측해 바꾸지 않고 다음 주문 변경 피드에서 확정 상태를 수집한다.
...
- 스마트스토어 연동이 꺼진 동안에는 수집 중인 실행이 없거나 시작 후 5분 이상 지났을 때 주문 조회 시작점을 현재 시각으로 옮기고 처리 기록을 비운다. 비활성 상태에서 활성 설정으로 재기동하면 애플리케이션 시작 시각부터 수집해 중지 기간 주문은 제외하고 첫 배치 전 주문은 포함한다. 5분 미만인 실행은 보호하며, 만료된 이전 실행이 조회 시작점을 되돌릴 수 없다.
...
- 스마트스토어 재고 동기화가 완료된 상품은 24시간마다 최대 100개씩 최신 절대 재고를 다시 전송한다. 외부 반영 직후 완료 기록을 남기지 못한 경우도 하루 안에 복구하며, 주문 수집 완료와 미반영 주문 확인 규칙은 동일하게 적용한다.
