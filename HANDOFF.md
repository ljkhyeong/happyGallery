...
- **환불 PG 호출 경계 (2026-07-12)**: 환불 요청 레코드는 `Propagation.MANDATORY`로 부모 트랜잭션에 참여하고 `RefundExecutionRequestedEvent`를 발행한다. `RefundExecutionEventListener`가 부모 커밋 이후 `refundExecutor`에서 `Propagation.NEVER`인 `RefundDispatcher`를 호출하며, 결과 업데이트와 재시도 검증은 `RefundTransactionService`의 짧은 `REQUIRES_NEW` 트랜잭션으로 처리한다.
