...
printf '%s\n' \
    ...
    'ROAD_ADDRESS_ENABLED=false' \
    'ROAD_ADDRESS_TIMEOUT_MILLIS=3000' \
    'ROAD_ADDRESS_CONNECT_TIMEOUT_MILLIS=1000' \
    'ROAD_ADDRESS_ACQUIRE_TIMEOUT_MILLIS=500' \
...
if printf '%s' "$output" \
        | grep -Eq '허용되지 않은 키.*(PAYMENT_TIMEOUT_MILLIS|TOSS_TIMEOUT_MILLIS|TOSS_CONNECT_TIMEOUT_MILLIS|TOSS_ACQUIRE_TIMEOUT_MILLIS|DELIVERY_TRACKING_ENABLED|DELIVERY_API_TIMEOUT_MILLIS|DELIVERY_API_CONNECT_TIMEOUT_MILLIS|DELIVERY_API_ACQUIRE_TIMEOUT_MILLIS|ROAD_ADDRESS_ENABLED|ROAD_ADDRESS_TIMEOUT_MILLIS|ROAD_ADDRESS_CONNECT_TIMEOUT_MILLIS|ROAD_ADDRESS_ACQUIRE_TIMEOUT_MILLIS|PUBLIC_HOLIDAY_ENABLED|PUBLIC_HOLIDAY_TIMEOUT_MILLIS|PUBLIC_HOLIDAY_CONNECT_TIMEOUT_MILLIS|PUBLIC_HOLIDAY_ACQUIRE_TIMEOUT_MILLIS)'; then
    printf '%s\n' "$output" >&2
    printf '결제·배송조회·주소·공휴일 운영 설정이 허용 목록에서 거부되었습니다.\n' >&2
    exit 1
fi
