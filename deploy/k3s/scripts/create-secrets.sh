...
validate_allowed_env_keys "$app_file" 애플리케이션 \
    ...
    TURNSTILE_ENABLED TURNSTILE_SITE_KEY TURNSTILE_SECRET_KEY TURNSTILE_HOSTNAME \
    ...
    ORDER_SHIPPING_FEE \
    NOTIFICATION_MODE NOTIFICATION_TIMEOUT_MILLIS \
...
# 심사 대기 중인 최초 배포만 disabled로 시작한다. 누락은 기존 NHN 모드다.
notification_mode=$(env_value NOTIFICATION_MODE "$app_file" 2>/dev/null || printf 'nhn')
case "$notification_mode" in
    nhn)
        for key in ALIMTALK_APP_KEY ALIMTALK_SECRET_KEY ALIMTALK_SENDER_KEY SMS_API_KEY SMS_API_SECRET SMS_SENDER_NUMBER; do
            require_env_value "$key" "$app_file" >/dev/null
        done
        ;;
    disabled)
        info "문자·알림톡 중지 모드: 휴대폰 인증과 고객 알림은 발송되지 않습니다."
        ;;
    *) die "NOTIFICATION_MODE는 nhn 또는 disabled여야 합니다." ;;
esac
