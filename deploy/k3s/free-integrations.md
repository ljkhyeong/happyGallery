...
| 택배사 | 공식 확인 내용 | 적용 판단 |
| --- | --- | --- |
| 우체국 | 공공데이터포털의 국내우편물 종적 조회 API는 무료. 개발·운영 자동승인, 개발계정 트래픽 10,000으로 안내 | 자동 갱신 추가 대상. 활용 승인·인증키·접속 방식 확인 후 구현 |
...
우체국 API는 13자리 등기번호로 배송 내역을 조회하는 XML 응답 방식이다. 웹훅 등록 API가 아니므로 연동을 추가할 때는 배송 중인 우체국 운송장만 배치로 조회하고, 배달 완료 건은 조회를 중단한다. 택배사가 제공한 배송 상황은 기존 배송 이력에 저장하며 주문 완료·적립·후기 요청은 자동 실행하지 않는다. 출처 표시 조건도 배송 정보 표시에 반영한다.

**현재 우체국 자동 갱신은 미구현이다.** 2026-09-12 공식 명세서의 요청 주소는 HTTP이고, 현재 개발 환경에서 같은 호스트의 HTTPS 연결은 5초 연결 제한 안에 성립하지 않았다. 인증키 없는 HTTP 요청은 XML 오류 응답을 반환했다. 이는 활용 승인된 정상 배송조회 검증이 아니다. 활용 신청 후 공식 지원 접속 주소·전송 보호 방식과 인증된 정상 응답을 확인한 다음 adapter와 조회 배치를 추가한다. 실제 인증키·운송장으로 평문 HTTP 호출을 하거나 공개 조회 화면을 크롤링하는 대체 연동은 추가하지 않았다.
