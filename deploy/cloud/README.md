...
2026-09-05 기준 클라우드 대안 비교다. 현재는 운영자가 허용한 보유 노트북의 Linux 설치·검증을 먼저 진행하며, **클라우드 VM 구매는 보류**한다. [노트북 설치 준비](../laptop/README.md)와 루트 `plan.md`를 먼저 따른다. 아래 견적·절차는 노트북 운영 조건을 충족하지 못할 때 다시 검토한다. 운영 VM·백업 저장소는 아직 생성하지 않았다. 클라우드 전환 조건은 [ADR-0049](../../docs/ADR/0049_저예산_클라우드_운영_기준/adr.md), 서비스 배포·복구 명령은 [k3s 운영 절차](../k3s/README.md)를 따른다.
