    ...
    public BatchResult syncChangedOrders() {
        ...
        try {
            ...
            if (!claimed.cursor().changedFrom().isBefore(changedTo)) {
                stateService.complete(claimed, new ChangeCursor(changedTo, null));
                return BatchResult.successOnly(0);
            }
            ...
            ChangeCursor next = page.nextCursor() == null
                    ...
                    : page.nextCursor();
            stateService.complete(claimed, next);
            return BatchResult.successOnly(latestChanges.size());
        } catch (Exception exception) {
            stateService.release(claimed);
            log.warn("스마트스토어 주문 동기화 실패 [type={}]", exception.getClass().getSimpleName());
            return BatchResult.of(0, Map.of("스마트스토어 주문을 가져오지 못함", 1));
        }
    }
