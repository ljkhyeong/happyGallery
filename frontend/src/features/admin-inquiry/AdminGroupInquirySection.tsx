...
export function AdminGroupInquirySection({ token, onAuthError }: Props) {
  ...
  const query = useAdminQuery(onAuthError, { queryKey: ["admin", "group-inquiries", status, paging.cursor],
    queryFn: () => listAdminGroupInquiries({ status: status || undefined, cursor: paging.cursor, size: 20 }, { headers: adminHeaders(token) }) });
    ...
    <div id="admin-group-inquiries">
      <div className="d-flex flex-wrap gap-2 justify-content-between mb-3">
        <Form.Group controlId="group-inquiry-status-filter"><Form.Label>문의 상태</Form.Label>
          <Form.Select value={status} onChange={(event) => { setStatus(event.target.value as GroupInquiryStatus | ""); paging.resetCursor(); }}>
...
function ConsultationForm({ detail, token, onAuthError }: Props & { detail: AdminGroupInquiryResponse }) {
  ...
  return <>
    ...
    {detail.activities.map((activity) => <div key={activity.id} className="border-top py-2 small">
      <strong>{GROUP_INQUIRY_STATUS[activity.toStatus]}</strong> · {formatDateTime(activity.createdAt)} · {activity.adminId === null ? "로컬 관리자" : `관리자 ${activity.adminId}`}
