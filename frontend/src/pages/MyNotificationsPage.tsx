...
import { useReadNotifications, useNotificationList, useUnreadCount } from "@/features/notification/useNotifications";
...
function NotificationInbox() {
  ...
  const pending = readMutation.isPending;
    ...
    <>
      <div className="d-flex flex-wrap justify-content-between gap-3 mb-3">
        ...
        <Button size="sm" variant="outline-secondary" disabled={pending || count.data === 0}
          onClick={() => read()}>{pending && readMutation.variables?.id === undefined ? "읽음 처리 중..." : "모두 읽음"}</Button>
      </div>
      <ErrorAlert error={query.error} onRetry={() => { void query.refetch(); }} />
      <ErrorAlert error={readMutation.error} onRetry={() => read(readMutation.variables?.id)} retrying={pending} />
            ...
            <div className="d-flex gap-3 justify-content-between align-items-start">
              <div className="flex-grow-1" style={{ minWidth: 0 }}>
                {target ? <Link to={target} onClick={() => {
                  if (!notification.read && !pending) readMutation.mutate({ id: notification.id, notifyOnError: true });
                }}>{label}</Link> : <strong>{label}</strong>}
